...
if( visit.patientPSH != undefined)
{
 ...
 var patientUpdatedPSH = await setPatientPSH(visit.patientPSH, patientobj);
 ...
 patientPshAsset =  await getAssetRegistry("mtbc.med.net.patientPSH.PatientPSH").then(async function (patientPshAsset){
  ...
  var checkpatientPSH = await query("getPatientPSH",{Id:patientobj.patientId});
  // IF PATIENT PAST SURGICAL HISTORY  ALREADY EXIST THEN UPDATE PATIENT FAMILY HISTORY STRCTURE
  
  if(checkpatientPSH.length > 0){
   return patientPshAsset.update(patientUpdatedPSH);
  }
  else{
   
    return patientPshAsset.add(patientUpdatedPSH);
  }
...
if( visit.patientPSHStructued != undefined)
{
 
 var patientUpdatedPSHstructured = await setPatientPSHstructured(visit.patientPSHStructued, patientobj);
 ...
 patientPshStructuredAsset =  await getAssetRegistry("mtbc.med.net.patientPSHStructued.PatientPSHStructued").then(async function (patientPshStructuredAsset){
  ...
  var checkpatientPSHStructured = await query("getPatientPSHStructued",{Id:patientobj.patientId});
  // IF PATIENT PAST SURGICAL HISTORY STRUCTURED ALREADY EXIST THEN UPDATE PATIENT FAMILY HISTORY STRCTURE
  
  if(checkpatientPSHStructured.length > 0){
   return patientPshStructuredAsset.update(patientUpdatedPSHstructured);
  }
  else{
   
    return patientPshStructuredAsset.add(patientUpdatedPSHstructured);
  }
...
if( visit.patientPastHosp != undefined)
{
 ...
 var patientUpdatedPastHosp = await setPatientPastHosp(visit.patientPastHosp, patientobj);
 ...
 patientPastHospitilizationAsset =  await getAssetRegistry("mtbc.med.net.patientPastHosp.PatientPastHosp").then(async function (patientPastHospitilizationAsset){
  ...
  var checkpatientPastHosp= await query("getPatientPastHosp",{Id:patientobj.patientId});
  // IF PATIENT PAST HOSPITILIZATION ALREADY EXIST THEN UPDATE PATIENT PAST HOSPITILIZATION
  
  if(checkpatientPastHosp.length > 0){
   
   return patientPastHospitilizationAsset.update(patientUpdatedPastHosp);
  }
  else{
    console.log(JSON.stringify(patientUpdatedPastHosp))
    return patientPastHospitilizationAsset.add(patientUpdatedPastHosp);
  }
...
// ==============================================================
   return patientParticipant,providerParticipant,pprAsset,patientAllergyAsset,patientImmunizationAsset,patientPmhAsset,patientFamilyHistoryStructureAsset,patientPshAsset,patientPshStructuredAsset,patientPastHospitilizationAsset;
 }
// ==============================================================
...
// ==============================================================
// PATIENT PAST SURGICAL HISTORY FUNCTION  START
// ==============================================================
async function setPatientPSH(patientPSH, patient)
{
 

 //CHECKING IS PATIENT PAST SURGICAL HISTORY IS NOT ALREADY UNDEFINED
 if( patientPSH != undefined)
 {
   //GETTING PATIENT PSH REGISTRY
   var patientPSHRegistry = await  getAssetRegistry('mtbc.med.net.patientPSH.PatientPSH');
   //CHECKING IF PATIENT PSH ALREADY EXIST IN REGISTY
   var checkPatientPSH = await patientPSHRegistry.exists(patient.patientId);
  
   if( checkPatientPSH )
   {
    //GETTIING ALL PRVIOUS PSh OF THE PATIENT
     var patientPriviousPSH = await query("getPatientPSH",{Id:patient.patientId});
     //TEMPRARY ARRAY THAT HOLD LATEST ADDIONS IN THE PSH BY THE PROVIDER
     var newPatientPSHlist = patientPSH.patientPSHhashs ;
     //ITERATING ALL THE PREVIOUS PSH OF THE PATIENT
      var newpatientPSHobject = patientPSH;
     patientPriviousPSH[0].patientPSHhashs.forEach(patientPSH => {
       // ITTERATION FOR NEW PSH FOR COMPARRING WITH PRIVIOUS ITTERATIONS
       newpatientPSHobject.patientPSHhashs.forEach( newPatientpsh => {
        //MATCHING  PSh OF OLD WITH NEW
         if( patientPSH.patientPSHId == newPatientpsh.patientPSHId )
         {
           // COPPYING ALL PREVIOUS VALUS WITH NEW
           patientPSH.patientPSHhash = newPatientpsh.patientPSHhash;
            //IF PATIENT PSH NOT ALRADY EXIST PUSH IN NEW PATIENT PSH LIST
            newPatientPSHlist = newPatientPSHlist.filter( el => el.patientPSHId !== newPatientpsh.patientPSHId  );
         }
       });
     });
   
     // PUSHING NEW PMH TO THE OLD RECORD
     newPatientPSHlist.forEach( _patientPSH =>{
      patientPriviousPSH[0].patientPSHhashs.push(_patientPSH);
     });
     // RETURNING PRVIOUS PATIENT PSH
     return patientPriviousPSH[0];
     ...
     // CREATING NEW PATIENT PMH FOR THIS PATIENT
     var factory = getFactory();
     var newPatientPSH = factory.newResource('mtbc.med.net.patientPSH', 'PatientPSH',patient.patientId);
     // ASSINING NEW PATIENT PMH HASH 
     newPatientPSH.patientPSHhashs = patientPSH.patientPSHhashs;
     return newPatientPSH;
   }
...
// ==============================================================
// PATIENT PAST SURGICAL STRUCTURED HISTORY FUNCTION  START
// ==============================================================
async function setPatientPSHstructured(patientPSHstructured, patient)
{

 //CHECKING IS PATIENT PAST SURGICAL HISTORY IS NOT ALREADY UNDEFINED
 if( patientPSHstructured != undefined)
 {
   //GETTING PATIENT PSH REGISTRY
   var patientPSHRegistry = await  getAssetRegistry('mtbc.med.net.patientPSH.PatientPSH');
   //CHECKING IF PATIENT PSH ALREADY EXIST IN REGISTY
   ...
   {
    //GETTIING ALL PRVIOUS PSh OF THE PATIENT
     var patientPriviousPSHstructured = await query("getPatientPSH",{Id:patient.patientId});
     //TEMPRARY ARRAY THAT HOLD LATEST ADDIONS IN THE PSH BY THE PROVIDER
     var newPatientPSHstructuredlist = patientPSHstructured.patientPSHStructuredhashs ;
     //ITERATING ALL THE PREVIOUS PSH OF THE PATIENT
     patientPriviousPSHstructured[0].patientPSHStructuredhashs.forEach(patientPSHsturctured => {
       // ITTERATION FOR NEW PSH FOR COMPARRING WITH PRIVIOUS ITTERATIONS
       patientPSHstructured.patientPSHStructuredhashs.forEach( newPatientpshStructured => {
        //MATCHING  PSh OF OLD WITH NEW
         if( patientPSHsturctured.patientPSHStructuredId == newPatientpshStructured.patientPSHStructuredId )
         {
  
           // COPPYING ALL PREVIOUS VALUS WITH NEW
           patientPSHsturctured.patientPSHStructuredhash = newPatientpshStructured.patientPSHStructuredhash;
            //IF PATIENT PSH NOT ALRADY EXIST PUSH IN NEW PATIENT PSH LIST
            newPatientPSHstructuredlist = newPatientPSHstructuredlist.filter( el => el.patientPSHStructuredId !== newPatientpsh.patientPSHStructuredId  );
         }
       });
     });
   
     // PUSHING NEW PMH TO THE OLD RECORD
     newPatientPSHstructuredlist.forEach( _patientPSHstructured =>{
      patientPriviousPSHstructured[0].patientPSHStructuredhashs.push(_patientPSHstructured);
     });
     // RETURNING PRVIOUS PATIENT PSH
     return patientPriviousPSHstructured[0];
     ...
     // CREATING NEW PATIENT PMH FOR THIS PATIENT
     var factory = getFactory();
     var newPatientPSHstructured = factory.newResource('mtbc.med.net.patientPSHStructued', 'PatientPSHStructued',patient.patientId);
     // ASSINING NEW PATIENT PMH HASH 
     newPatientPSHstructured.patientPSHStructuredhashs = patientPSHstructured.patientPSHStructuredhashs;
     return newPatientPSHstructured;
   }
...
// ==============================================================
// PATIENT PAST SURGICAL HISTORY STRUCTURED FUNCTION  START
// ==============================================================


// ==============================================================
// PATIENT PAST HOSPITILIZATION FUNCTION  START
// ==============================================================
async function setPatientPastHosp(patientPastHosp, patient)
{

 //CHECKING IS PATIENT PAST Hosp IS NOT ALREADY UNDEFINED
 if( patientPastHosp != undefined)
 {
   //GETTING PATIENT PAST Hosp REGISTRY
   var patientPastHospRegistry = await  getAssetRegistry('mtbc.med.net.patientPastHosp.PatientPastHosp');
   //CHECKING IF PATIENT PAST HOSP ALREADY EXIST IN REGISTY
   var checkPatientPastHosp = await patientPastHospRegistry.exists(patient.patientId);
   if( checkPatientPastHosp )
   {
    //GETTIING ALL PRVIOUS PAST HOSP OF THE PATIENT
     var patientPriviousPastHosp = await query("getPatientPastHosp",{Id:patient.patientId});
     //TEMPRARY ARRAY THAT HOLD LATEST ADDIONS IN THE PAST HOSP BY THE PROVIDER
     var newPatientPastHosplist = patientPastHosp.patientPastHosphashs ;
     //ITERATING ALL THE PREVIOUS PAST HOSP OF THE PATIENT
     patientPriviousPastHosp[0].patientPastHosphashs.forEach(patientpreHosp => {
       // ITTERATION FOR NEW PATIENT PAST HOSPITILIZAITON FOR COMPARRING WITH PRIVIOUS ITTERATIONS
       patientPastHosp.patientPastHosphashs.forEach( newPatientPastHosp => {
        //MATCHING  PAST HOSPITILIZATION OF OLD WITH NEW
         if( patientpreHosp.patientPastHospId == newPatientPastHosp.patientPastHospId )
         {
  
           // COPPYING ALL PREVIOUS VALUS WITH NEW
           patientpreHosp.patientPastHosphash = newPatientPastHosp.patientPastHosphash;
            //IF PATIENT PAST HOSPITILIZAITON NOT ALRADY EXIST PUSH IN NEW PATIENT PAST HOSPITILIZATION LIST
            newPatientPastHosplist = newPatientPastHosplist.filter( el => el.patientPastHospId !== newPatientPastHosp.patientPastHospId  );
         }
       });
     });
   
     // PUSHING NEW PAST HOSPITILIATION TO THE OLD RECORD
     newPatientPastHosplist.forEach( _patientPasthosp =>{
      patientPriviousPastHosp[0].patientPastHosphashs.push(_patientPasthosp);
     });
     // RETURNING PRVIOUS PATIENT PAST HOSPITILIZAIOTN
     return patientPriviousPastHosp[0];

      

   }else{
     // CREATING NEW PATIENT PAT HOSPITILIZATION FOR THIS PATIENT
     var factory = getFactory();
     var newPatientPastHosp= factory.newResource('mtbc.med.net.patientPastHosp', 'PatientPastHosp',patient.patientId);
     // ASSINING NEW PATIENT PAST HOSPITILIZAIOTN HASH 
     newPatientPastHosp.patientPastHosphashs = patientPastHosp.patientPastHosphashs;
     return newPatientPastHosp;
   }
...
// ==============================================================
// PATIENT PAST HOSPITILIZAITON FUNCTION  START
// ==============================================================
